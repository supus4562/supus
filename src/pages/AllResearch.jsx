      ...
      <div style={{ padding: '6rem 4rem', maxWidth: '1400px', margin: '0 auto' }}>
        {researchData.length === 0 ? (
          <ProBox bg={T.bg} style={{ padding: '4rem', textAlign: 'center', border: `4px dashed #fff` }}>
            <h3 style={{ fontFamily: T.fontDisplay, fontSize: 'clamp(1.8rem, 6vw, 2.5rem)', wordBreak: 'break-word', lineHeight: 1.1, textTransform: 'uppercase', margin: '0 0 1rem', color: '#fff' }}>NO PUBLISHED PAPERS.</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 400px), 1fr))', gap: '3rem' }}>
            {researchData.map((res, i) => (
              <ProBox key={i} delay={0.1*i} bg={T.bgSecondary} onClick={() => res.link ? window.open(res.link, '_blank') : null}>
                <h3 style={{ fontFamily: T.fontDisplay, fontWeight: 800, fontSize: 'clamp(1.5rem, 5vw, 2rem)', wordBreak: 'break-word', lineHeight: 1.1, textTransform: 'uppercase', margin: '0 0 1rem' }}>{res.title}</h3>
